...
public class WifiDirectService extends Service  implements WifiP2pManager.ChannelListener {
    ...
    private Boolean weAreRunning = false;
    ...
    public void onChannelDisconnected() {
        ...
        mySpeech.speak("I lost the Channel ");
        sendResult("I lost the Channel ");
    }
    ...
    private class PeerReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if(WIFI_P2P_PEERS_CHANGED_ACTION.equals(action)) {
                ...
                String persTatu = "Peer discovery state changed to ";
                ...
                mySpeech.speak(persTatu);
                sendResult(persTatu);


                if(state == WifiP2pManager.WIFI_P2P_DISCOVERY_STOPPED && weAreRunning){
                    p2p.discoverPeers(channel, new WifiP2pManager.ActionListener() {
                        public void onSuccess() {
                            sendResult("Re-Started peer discovery");
                        }

                        public void onFailure(int reason) {
                            sendResult("Re-Starting peer discovery failed, error code " + reason);
                        }
                    });
                }
